...
| B1 | 유니온 멤버 인접 | `lib/similarity.ts` | [x] 정렬 제약으로 재구현 — 아래 참조 |

**B1 경위.** 처음에는 웹처럼 무게중심을 당기는 힌트로 넣었다(pull 0.1/0.25/0.5, "마지막
스윕에만 적용" 변형 포함). 형제 거리는 4% 줄지만 최장 엣지가 10% 늘어 되돌렸다. 이후
"유니온은 왼쪽에 부모, 오른쪽에 배리언트" 라는 요구를 받고 **정렬 제약**으로 다시 만들었다:
스윕이 끝난 뒤 클러스터를 한 덩어리로 모으고, 블록 단위 transpose 로 다시 정련하고, 높이
균형이 덩어리를 쪼개지 않게 한다. 완전히 붙은 유니온 0 → 13/42, world 높이 8% 단축, 평균
엣지 길이는 그대로, 교차 +7.5%. 배리언트를 부모 랭크+1로 **강제**하는 변형은 18/42까지
올라가지만 길이 +14%·교차 +35% 라 채택하지 않았다. 자세한 수치는 PORT.md 참조.
