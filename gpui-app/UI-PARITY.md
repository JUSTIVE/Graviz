...
### 남은 것
- 3. 사이드바 폭 드래그 리사이즈/영속화 (현재 340px 고정, 접기/펼치기만)
- 5.4 포커스 엣지 위젯(하단 중앙) — 엣지 호버 툴팁으로 대체 중
- 5.6 레이아웃 진행 오버레이 — 네이티브는 즉시라 불필요 판단
- 오버레이 도크 높이 드래그 리사이즈, CodeMirror 수준의 문법 하이라이트
- 카드 포커스 리플 애니메이션, 모바일 레이아웃(네이티브 비해당)
