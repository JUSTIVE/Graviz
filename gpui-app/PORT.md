...
지표는 셋을 같이 본다: `avg len`, `crossings`(엣지가 지나가는 남의 카드 수), `edge-x`(엣지간
기하 교차, 세그먼트를 격자에 넣고 셈). 우선순위는 **엣지간 교차 > 카드 교차 > 길이**.

| 변경 | avg len | card/엣지 | edge-x/엣지 |
|---|---|---|---|
| 유니온 인접까지 | 4716 | 4.60 | — |
| 랭크 내 빈틈 압축 | 4385 | 6.01 | 190.0 |
| **압축 되돌림 + 스무딩 80 + 동점교환 + 시프팅** | **4580** | **4.03** | **137.4** |
